...
// Earnings confirmados. Editar acá cuando cambien.
const CATALYSTS = {
...
const ENT={'&amp;':'&','&lt;':'<','&gt;':'>','&quot;':'"','&#39;':"'",'&apos;':"'","&nbsp;":" "};
const clean = s => (s||'').replace(/<!\[CDATA\[|\]\]>/g,'').replace(/&#(\d+);/g,(_,d)=>String.fromCharCode(+d)).replace(/&[a-z]+;/gi,m=>ENT[m]!==undefined?ENT[m]:m).trim();

async function get(url, timeout=10000){
  const c=new AbortController(); const id=setTimeout(()=>c.abort(),timeout);
  try{
    const r=await fetch(url,{signal:c.signal,headers:{'User-Agent':'Mozilla/5.0 (compatible; InversionesBot/1.0)'}});
    clearTimeout(id);
...
function parseRSS(xml){
  ...
  while((m=re.exec(xml))!==null){
    ...
    const src  =clean((it.match(/<source[^>]*>([\s\S]*?)<\/source>/)||[])[1]) || 'Yahoo Finance';
...
// Targets y consenso REALES desde Yahoo quoteSummary
